...
reCAPTCHA Enterprise で「本物のブラウザで動いている、このアプリからのリクエストか」を
確かめ、スクリプトが家族を作りつづけるような使われかたを止める。

クラシックの reCAPTCHA v3 ではなく Enterprise なのは、Firebase コンソールが v3 を
**非推奨**と表示したうえで秘密鍵を要求し、いまの鍵作成フローがその秘密鍵を発行しない
ため。新しく作れるのは Cloud 管理の Enterprise 鍵だけで、こちらはサイトキーのみで動く。

1. Google Cloud コンソール → セキュリティ → reCAPTCHA で **ウェブ / スコアベース**の
   鍵を作る。許可ドメインに本番のホスト名、`tsumiki-preview.vercel.app`（固定
   プレビュー）、`localhost` を入れる。先に reCAPTCHA Enterprise API を有効に
   しておくこと（していないと作成が `リクエストが無効です` で落ちる）。
...
4. **しばらくは「適用しない」のままにする。** App Check の画面で、検証済みリクエストの
   ...
   Authentication の適用は当面オフのままにする — そこで設定を誤るとサインインごと
   不能になり、原因を見に行くこともできなくなる。

### トークンの有効期間はコストそのもの

reCAPTCHA Enterprise は**月 10,000 アセスメントまで無料**で、超過分は課金される
（このプロジェクトは Blaze）。アセスメントが発生するのは App Check トークンを
作り直すときだけで、その間隔が App Check の TTL。

TTL 1 時間だと 1 端末あたり月 700 回を超え、無料枠は十数台で尽きる。**TTL は 1 日**に
してある（Firebase コンソールの App Check → アプリ → トークンの有効期間）。これで
1 端末あたり月 30 回ほどになり、無料枠で数百台まで持つ。縮めるときは、それが
そのまま請求額になることを承知のうえで。
